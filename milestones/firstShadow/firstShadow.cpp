...
int main() {
    Camera cam(Point(-0.5, 1, -1), Vec(0.2, -0.1, 1), Vec(0, 1, 0), 200, 400, 0.013, 0.0001);
    World world(PointLight(Point(0, 2, 0), Colour(1, 1, 1)));
    world.addMaterial(Material(0.1, 0.9, 0.9, 100, Colour(0, 0.5, 0.8)));
    world.addMaterial(Material(0.2, 0.8, 0.6, 100, Colour(0, 0.5, 0.4)));
    world.addMaterial(Material(0.1, 0.9, 0.9, 100, Colour(0, 0.4, 0.5)));
    world.addObject(TSphere(TranslationMat(0, 0, 2)), 0);
    world.addObject(Cube(TranslationMat(0, -1, 2)*ScalingMat(1000, 0.2, 1000)), 1);
    world.addObject(Cube(TranslationMat(0, 0, 8)*ScalingMat(1000, 1000, 0.1)), 2);
    PhongShader shader;
    std::cout << "Setup complete" << std::endl;
    cam.render(world, shader);
    std::cout << "Image created" << std::endl;
    cam.save("/home/william/CPP/Projects/RayTracer2/milestones/firstShadow/firstShadow.ppm");
}